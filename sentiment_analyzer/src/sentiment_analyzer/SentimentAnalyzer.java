...
public class SentimentAnalyzer {
	...
	public static void main(String[] args) throws IOException {
		...
		if (!dynamoHelper.checkIfTableExists(TWEET_TABLE_NAME)) {
			...
			// create table
			dynamoHelper.createTable(attributeDefinitions, keySchemaElements, 10L, 10L, TWEET_TABLE_NAME, localSecondaryIndexes);
		}
		...
		if (taggedTweetsQueueUrl == null) {
			System.out.println("There is no tagged tweets queue");
			System.exit(1);
		}
		...
		while (true) {
			// get tagged tweets
			List<Message> messages = sqsHelper.receiveMessage(taggedTweetsQueueUrl);
				...
				try {
					...
					if (score != 0.0) {
						...
						for (int i=0; i<keywords.length(); i++) {
							...
							dynamoHelper.putItem(TWEET_TABLE_NAME, attrMap);
						}
					}
						
					// deletes the message
					sqsHelper.deleteMessage(taggedTweetsQueueUrl, message.getReceiptHandle());
				} 
